      ...
      >
        ...
        >
          <div class="flex items-center justify-between space-x-4">
            <span class="engraved-org text-xl">{{ auth.user?.role_id === 0 ? 'Super Admin' : (profileStore.userProfile.company ?? '...') }}</span>
...
function makeOrgLink(path: string) {
  try {
    const q = route && route.query ? (route.query.org || route.query.org_id ? { org: String(route.query.org || route.query.org_id) } : {}) : {}
